...
_whatweb_search_path = ('whatweb', '/usr/bin/whatweb', '/usr/local/bin/whatweb', '/opt/local/bin/whatweb')
...
class WhatWeb(object):
    ...
    def _ensure_path_and_version(self):
        ...
        for whatweb_path in self._search_path:
            proc = None
            try:
                proc = yield from asyncio.create_subprocess_exec(whatweb_path, '--version', stdout = asyncio.subprocess.PIPE)
                while True:
